...
def generate_mcqs(skills):
    question_templates = [
        ...
        "Identify a common challenge in using {skill}.",
        "Choose an application of {skill} from the following."
    ]
    option_templates = [
        "{skill} Basics",
        "{skill} Advanced Use",
    ...
    for category, count in question_distribution.items():
        ...
        for i, skill in enumerate(skill_set[:count]):
            question = question_templates[i % len(question_templates)].format(skill=skill)
            options = [opt.format(skill=skill) for opt in option_templates]
            shuffle(options)
...
'''def generate_mcqs(skills):
    ...
    for category, count in question_distribution.items():
        skill_set = skills.get(category, SKILL_ONTOLOGY.get(category, []))
        for i, skill in enumerate(skill_set[:count]):
            question = f"What is a common use of {skill}?"
            options = [f"{skill} Basics", f"{skill} Advanced Use", f"{skill} Challenges", f"{skill} Applications"]
            shuffle(options)
            mcqs[category].append({
                "id": f"{category}_{i + 1}",
                "question": question,
...
def start_test():
    # Mocked extracted skills
    extracted_skills = {
        "technical": ["Python", "SQL", "Django"],
        "soft": ["Communication", "Teamwork"],
        "managerial": ["Leadership"]
    }

    mcqs = generate_mcqs(extracted_skills)
    session['mcqs'] = mcqs  # Save MCQs in session for later scoring
...
def submit_test():
    ...
    return render_template('score.html', score=score, total=total, percentage=round(percentage, 2))'''
@app.route('/submit_test', methods=['POST'])
def submit_test():
    answers = request.form
    mcqs = session.get('mcqs', {})
    ...
    percentage = (score / total) * 100 if total > 0 else 0
    result = {"score": score, "total": total, "percentage": round(percentage, 2)}
